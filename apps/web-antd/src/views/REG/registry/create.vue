        ...
        <div v-show="currentStep === 0">
          <Form layout="vertical" class="max-w-lg">
            <FormItem label="仓库名称" required>
              <Input
                v-model:value="formData.registryName"
                placeholder="请输入仓库名称"
              />
            </FormItem>
            <FormItem label="仓库类型">
          ...
          <Form layout="vertical" class="max-w-lg">
            ...
            <template v-if="formData.deployMode === 'ssh'">
              <FormItem label="主机地址" required>
                <Input
                  ...
                  placeholder="IP 地址或主机名"
                />
              </FormItem>
              <FormItem label="SSH 端口">
                <InputNumber
                  v-model:value="formData.sshPort"
                  :min="1"
                  :max="65535"
                  style="width: 100%"
                />
              ...
              <FormItem label="SSH 密码">
                <Input.Password
                  v-model:value="formData.encryptedPassword"
                  placeholder="请输入密码"
                />
              </FormItem>
              <FormItem label="服务端口">
                <Space>
                  <InputNumber
                    ...
                    style="width: 200px"
                  />
                  <Button :loading="portChecking" @click="handleCheckPort"
                    >检测端口</Button
                  >
                </Space>
                <div v-if="portCheckResult" class="mt-1 text-xs">
                  <span v-if="portCheckResult.error" class="text-red-500">{{
                    portCheckResult.error
                  }}</span>
                  <span
                    v-else-if="portCheckResult.occupied"
              ...
              <FormItem label="安装路径">
                <Input
                  v-model:value="formData.installPath"
            ...
            <template v-if="formData.deployMode === 'k8s'">
              ...
              <FormItem label="Helm Values (YAML)">
                <Textarea
                  v-model:value="formData.helmValues"
                  :rows="6"
                  placeholder="自定义 Helm values.yaml 内容"
                />
              </FormItem>
            </template>
          </Form>
          ...
          <Form layout="vertical" class="max-w-lg">
            <FormItem label="管理员用户名">
              <Input
                ...
                :disabled="
                  formData.registryType === 'harbor' ||
                  formData.registryType === 'gitlab'
                "
                :placeholder="
                  formData.registryType === 'gitlab' ? 'root' : 'admin'
                "
              />
              <div class="mt-1 text-xs text-gray-400">
                <span v-if="formData.registryType === 'harbor'"
                  >Harbor 管理员用户名固定为 admin，不可更改</span
                >
                <span v-else-if="formData.registryType === 'gitlab'"
                  >GitLab 管理员用户名固定为 root，不可更改</span
                >
            ...
            <FormItem label="管理员密码" required>
              <Input.Password
                v-model:value="formData.encryptedAdminPassword"
            ...
            <FormItem label="访问地址">
              <Input
                ...
                placeholder="http://host:port (可选，自动生成)"
              />
            </FormItem>
          </Form>
        </div>
        ...
        <div v-show="currentStep === 3">
          <div class="max-w-lg space-y-2">
            ...
            <p>
              <strong>部署方式:</strong>
              {{
                formData.deployMode === 'ssh' ? 'SSH 远程部署' : 'K8s 集群部署'
              }}
            </p>
            <template v-if="formData.deployMode === 'ssh'">
              <p>
                <strong>目标主机:</strong> {{ formData.host }}:{{
                  formData.sshPort
                }}
              </p>
              <p>
                <strong>服务端口:</strong>
                {{ formData.servicePort || getDefaultPort() }}
              </p>
              <p>
                <strong>安装路径:</strong>
                {{ formData.installPath || getDefaultInstallPath() }}
              </p>
            </template>
            <template v-if="formData.deployMode === 'k8s'">
