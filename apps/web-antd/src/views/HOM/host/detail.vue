...
function memPercent() {
  ...
  return Math.round(
    (Number(host.value.usedMemGb) / Number(host.value.totalMemGb)) * 100,
  );
}
...
function diskPercent() {
  ...
  return Math.round(
    (Number(host.value.usedDiskGb) / Number(host.value.totalDiskGb)) * 100,
  );
}
      ...
      <Card :loading="loading">
        ...
        <template v-if="host">
          <Descriptions bordered :column="2">
            ...
            <DescriptionsItem label="SSH端口">{{
              host.sshPort
            }}</DescriptionsItem>
            <DescriptionsItem label="SSH用户">{{
              host.sshUser
            }}</DescriptionsItem>
            <DescriptionsItem label="操作系统"
              >{{ host.osType || '-' }}
              {{ host.osVersion || '' }}</DescriptionsItem
            >
            ...
            <DescriptionsItem label="机架">{{
              host.rack || '-'
            }}</DescriptionsItem>
            <DescriptionsItem label="节点标签">{{
              host.nodeLabel || '-'
            }}</DescriptionsItem>
            <DescriptionsItem label="状态">
              <Tag :color="statusColorMap[host.status] || 'default'">{{
                host.status
              }}</Tag>
            </DescriptionsItem>
            <DescriptionsItem label="Agent状态">
              <Tag
                :color="host.agentStatus === 'running' ? 'green' : 'default'"
                >{{ host.agentStatus }}</Tag
              >
            </DescriptionsItem>
