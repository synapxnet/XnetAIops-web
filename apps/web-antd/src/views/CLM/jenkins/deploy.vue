...
const addGitCredential = () => {
  if (!deployConfig.value.gitCredentials)
    deployConfig.value.gitCredentials = [];
...
const addHarborCredential = () => {
  if (!deployConfig.value.harborCredentials)
    deployConfig.value.harborCredentials = [];
...
const addSshCredential = () => {
  if (!deployConfig.value.sshCredentials)
    deployConfig.value.sshCredentials = [];
  ...
  try {
    ...
    if (versions.length > 0) {
      ...
      if (!deployConfig.value.jenkinsVersion) {
        const latest =
          versions.find((v: JenkinsVersion) => v.isLatest) || versions[0];
...
const handleRefreshVersions = async () => {
  ...
  try {
    ...
    message.error('刷新版本列表失败: ' + e.message);
  } finally {
...
const testConnection = async () => {
  ...
  try {
    ...
    if (result.success) {
      ...
      if (
        result.detectedOsType &&
        result.detectedOsType !== masterForm.value.os_type
      ) {
...
const previewScript = async () => {
  try {
    const res = (await previewMasterDeployScript(
      masterForm.value.os_type || 'linux',
      deployConfig.value,
    )) as any;
...
const executeDeploy = async () => {
  ...
  try {
    ...
    deployConfig.value.jenkinsHome =
      masterForm.value.jenkins_home || '/var/jenkins_home';
    deployConfig.value.javaVersion = masterForm.value.java_version || '17';
    deployConfig.value.adminUsername =
      masterForm.value.admin_username || 'admin';
    deployConfig.value.adminPassword =
      masterForm.value.admin_password || deployConfig.value.adminPassword;

    const res = (await deployJenkinsMaster(
      masterId.value!,
      deployConfig.value,
    )) as any;
    const result = res?.data || res;
...
const nextStep = async () => {
  if (currentStep.value === 0) {
    ...
    if (
      !deployConfig.value.jenkinsVersion ||
      !deployConfig.value.adminPassword
    ) {
...
const goBack = () => {
  router.push('/CLM/jenkins/list');
};
const openJenkins = () => {
  window.open(
    `http://${masterForm.value.host}:${deployConfig.value.jenkinsPort}`,
    '_blank',
  );
};
const copyPassword = () => {
  navigator.clipboard
    .writeText(deployConfig.value.adminPassword || '')
    .then(() => {
      ...
      <Card>
        ...
        <template #title>
          <Space>
            ...
            ></Button>
            <span>{{
              masterId ? '编辑Master配置' : '部署Jenkins Master'
            }}</span>
          </Space>
        </template>
              ...
              <Form :label-col="{ span: 4 }" :wrapper-col="{ span: 16 }">
                <Form.Item label="Master名称" required>
                  <Input
                    ...
                    placeholder="如: jenkins-master-01"
                  />
                </Form.Item>
                ...
                <template v-if="connectionMode === 'select'">
                  <Form.Item label="选择主机" required>
                    <Spin :spinning="loadingHosts">
                      ...
                      >
                        <Select.Option
                          ...
                          :key="h.id"
                          :value="h.id"
                          :label="`${h.hostname} (${h.ipAddress})`"
                        >
                          {{ h.hostname || '-' }} ({{ h.ipAddress }})
                          <Tag
                            v-if="h.status"
                            :color="h.status === 'online' ? 'green' : 'default'"
                            size="small"
                    ...
                    <div
                      v-if="homHosts.length === 0 && !loadingHosts"
                      style="margin-top: 8px"
                    >
                      <Alert
                        type="warning"
                        message="暂无主机，请先在主机管理(HOM)中添加"
                        show-icon
                      />
                    </div>
                  </Form.Item>
                ...
                <template v-else>
                  <Form.Item label="主机地址" required>
                    <Input
                      v-model:value="masterForm.host"
                      placeholder="IP地址或域名"
                    />
                  </Form.Item>
                  <Form.Item label="SSH端口" required>
                    <InputNumber
                      v-model:value="masterForm.port"
                      :min="1"
                      :max="65535"
                      style="width: 120px"
                    />
                  ...
                  <Form.Item label="用户名" required>
                    <Input
                      v-model:value="masterForm.username"
                      placeholder="SSH用户名"
                    />
                  </Form.Item>
                  <Form.Item label="密码" required>
                    <Input.Password
                      v-model:value="masterForm.password"
                      placeholder="SSH密码"
                    />
                  </Form.Item>
                  <Form.Item label="操作系统">
                  ...
                  <Form.Item label="地域">
                    <Select
                      v-model:value="masterForm.region"
                ...
                <Form.Item label="描述">
                  <Input.TextArea
                    v-model:value="masterForm.description"
                    :rows="2"
                    placeholder="可选的描述信息"
                  />
                </Form.Item>
                ...
                <div v-if="testResult" class="test-result">
                  ...
                  <div v-if="testResult.success" class="server-info">
                    ...
                    >
                      ...
                      <Descriptions.Item
                        v-if="testResult.availableDisk"
                        label="可用磁盘"
                        >{{ testResult.availableDisk }}</Descriptions.Item
                      >
                      <Descriptions.Item
                        v-if="testResult.totalMemoryMb"
                        label="总内存"
                        >{{ testResult.totalMemoryMb }}MB</Descriptions.Item
                      >
                    </Descriptions>
                  </div>
                </div>
            ...
            <Card title="Jenkins配置" :bordered="false">
              <Form :label-col="{ span: 4 }" :wrapper-col="{ span: 16 }">
                <Form.Item label="Jenkins版本" required>
                  ...
                  <div class="hint-text">
                    版本列表从镜像站获取，点击刷新可获取最新版本
                  </div>
                </Form.Item>
                <Form.Item label="HTTP端口" required>
                  <InputNumber
                    v-model:value="deployConfig.jenkinsPort"
                    :min="1"
                    :max="65535"
                    style="width: 120px"
                  />
                ...
                <Form.Item label="Jenkins Home">
                  <Input
                    v-model:value="deployConfig.jenkinsHome"
                ...
                <Form.Item label="JVM参数">
                  <Input
                    v-model:value="deployConfig.javaOpts"
                ...
                <Form.Item label="管理员用户名" required>
                  <Input
                    v-model:value="deployConfig.adminUsername"
                    placeholder="admin"
                  />
                </Form.Item>
                <Form.Item label="管理员密码" required>
                  <Input.Password
                    v-model:value="deployConfig.adminPassword"
                    placeholder="设置管理员密码"
                  />
                </Form.Item>
                <Form.Item label="管理员邮箱">
                  <Input
                    v-model:value="deployConfig.adminEmail"
                    placeholder="admin@localhost"
                  />
                </Form.Item>
                <Form.Item label="安装推荐插件">
                  <Switch
                    v-model:checked="deployConfig.installSuggestedPlugins"
                  />
                </Form.Item>
              </Form>
            </Card>
            ...
            <Card title="凭证配置 (可选)" :bordered="false">
              ...
              <Collapse :default-active-key="['git']">
                ...
                <Collapse.Panel key="git" header="Git凭证配置">
                  <div
                    v-for="(cred, index) in deployConfig.gitCredentials"
                    :key="index"
                  ...
                  >
                    <Form :label-col="{ span: 4 }" :wrapper-col="{ span: 18 }">
                      <Form.Item label="凭证ID"
                        ><Input
                          v-model:value="cred.id"
                          placeholder="如：gitee-synap-xnet"
                      /></Form.Item>
                      <Form.Item label="描述"
                        ><Input
                          v-model:value="cred.description"
                          placeholder="凭证描述"
                      /></Form.Item>
                      <Form.Item label="用户名"
                        ><Input
                      ...
                      <Form.Item :wrapper-col="{ offset: 4 }"
                        ><Button
                          type="link"
                          danger
                          @click="removeGitCredential(index)"
                          >删除此凭证</Button
                        ></Form.Item
                      >
                    </Form>
                    <Divider
                      v-if="
                        index < (deployConfig.gitCredentials?.length || 0) - 1
                      "
                    />
                  </div>
                  <Button
                ...
                <Collapse.Panel key="harbor" header="Harbor凭证配置">
                  <div
                    v-for="(cred, index) in deployConfig.harborCredentials"
                    :key="index"
                    class="credential-item"
                  >
                    <Form :label-col="{ span: 4 }" :wrapper-col="{ span: 18 }">
                      ...
                      <Form.Item label="Harbor URL"
                        ><Input
                          v-model:value="cred.url"
                          placeholder="如：https://harbor.example.com"
                      /></Form.Item>
                      <Form.Item label="用户名"
                        ><Input
                          v-model:value="cred.username"
                      ...
                      <Form.Item :wrapper-col="{ offset: 4 }"
                        ><Button
                          type="link"
                          danger
                          @click="removeHarborCredential(index)"
                          >删除此凭证</Button
                        ></Form.Item
                      >
                    </Form>
                    <Divider
                      v-if="
                        index <
                        (deployConfig.harborCredentials?.length || 0) - 1
                      "
                    />
                  </div>
                  <Button
                ...
                <Collapse.Panel key="ssh" header="SSH凭证配置">
                  <div
                    v-for="(cred, index) in deployConfig.sshCredentials"
                    :key="index"
                    class="credential-item"
                  >
                    <Form :label-col="{ span: 4 }" :wrapper-col="{ span: 18 }">
                      ...
                      <Form.Item label="私钥"
                        ><Input.TextArea
                          v-model:value="cred.privateKey"
                          :rows="4"
                          placeholder="粘贴SSH私钥内容"
                      /></Form.Item>
                      <Form.Item label="私钥密码"
                        ><Input.Password
                          v-model:value="cred.passphrase"
                          placeholder="私钥密码（可选）"
                      /></Form.Item>
                      <Form.Item :wrapper-col="{ offset: 4 }"
                        ><Button
                          type="link"
                          danger
                          @click="removeSshCredential(index)"
                          >删除此凭证</Button
                        ></Form.Item
                      >
                    </Form>
                    <Divider
                      v-if="
                        index < (deployConfig.sshCredentials?.length || 0) - 1
                      "
                    />
                  </div>
                  <Button
            ...
            <Card title="执行部署" :bordered="false">
              <div v-if="!deploying && !deploySuccess">
                ...
                >
                  ...
                  <Descriptions.Item label="Jenkins端口">{{
                    deployConfig.jenkinsPort
                  }}</Descriptions.Item>
                  <Descriptions.Item label="Jenkins版本">{{
                    deployConfig.jenkinsVersion
                  }}</Descriptions.Item>
                  <Descriptions.Item label="Java版本">{{
                    deployConfig.javaVersion
                  }}</Descriptions.Item>
                  <Descriptions.Item label="管理员用户">{{
                    deployConfig.adminUsername
                  }}</Descriptions.Item>
                  <Descriptions.Item label="Jenkins Home">{{
                    deployConfig.jenkinsHome
                  }}</Descriptions.Item>
                </Descriptions>
                    ...
                    <div class="success-credentials">
                      ...
                      >
                        <Descriptions.Item label="访问地址">
                          <a
                            :href="`http://${masterForm.host}:${deployConfig.jenkinsPort}`"
                            target="_blank"
                          >
                            http://{{ masterForm.host }}:{{
                              deployConfig.jenkinsPort
                            }}
                          </a>
                        </Descriptions.Item>
                          ...
                          <Space>
                            <code
                              style="
                                ...
                                color: #1890ff;
                              "
                              >{{ deployConfig.adminPassword }}</code
                            >
                            <Button size="small" @click="copyPassword"
                              >复制</Button
                            >
                          </Space>
                        </Descriptions.Item>
                      ...
                      <Alert
                        type="info"
                        message="请妥善保管以上登录信息。如果部署脚本已自动完成初始化，可直接使用上述账号密码登录。"
                        show-icon
