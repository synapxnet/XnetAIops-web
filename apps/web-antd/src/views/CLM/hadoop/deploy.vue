...
async function handleDeploy() {
  if (!createdId.value) {
    message.error('请先保存服务器信息');
    return;
  }
  deploying.value = true;
  try {
    const res = (await deployHadoopCluster(
      createdId.value,
      deployConfig,
    )) as any;
      ...
      <Card title="部署 Hadoop 集群">
              ...
              <Col :span="12">
                <FormItem label="节点名称" required>
                  <Input
                    ...
                    placeholder="如: hadoop-master-01"
                  />
                </FormItem>
              </Col>
              <Col :span="12">
            ...
            <Row :gutter="16">
              ...
              <Col :span="12">
                <FormItem label="SSH密码" required>
                  <Input.Password
                    v-model:value="serverForm.sshPassword"
                    placeholder="SSH登录密码"
                  />
                </FormItem>
              </Col>
          ...
          <Form layout="vertical" :model="deployConfig">
            ...
            <Row :gutter="16">
              <Col :span="8">
                <FormItem label="副本因子">
                  <InputNumber
                    v-model:value="deployConfig.hdfsReplication"
                    :min="1"
                    :max="10"
                    style="width: 100%"
                  />
              ...
              <Col :span="8">
                <FormItem label="YARN内存(MB)">
                  <InputNumber
                    v-model:value="deployConfig.yarnMemory"
                    :min="1024"
                    :step="1024"
                    style="width: 100%"
                  />
              ...
              <Col :span="8">
                <FormItem label="YARN CPU核数">
                  <InputNumber
                    v-model:value="deployConfig.yarnCpu"
                    :min="1"
                    :max="128"
                    style="width: 100%"
                  />
        ...
        <div v-if="currentStep === 2" style="text-align: center; padding: 40px">
          <Alert
            ...
            show-icon
            message="部署任务已提交"
            description="Hadoop部署脚本正在远程执行，您可以前往列表查看进度和日志。"
          />
