...
const stats = computed(() => {
  ...
  return {
    ...
    running: all.filter(
      (i) => i.status === 'running' || i.status === 'deployed',
    ).length,
  };
});
        ...
        >
          <template #bodyCell="{ column, record }">
            <template v-if="column.key === 'nodeType'">
              <Tag :color="nodeTypeColors[record.nodeType] || 'default'">{{
                record.nodeType
              }}</Tag>
            </template>
