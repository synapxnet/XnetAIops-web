...
import type { MySQLInstance } from '../api/types';
...
const columns = [
  ...
  { title: '端口', dataIndex: 'mysqlPort', key: 'mysqlPort', width: 80 },
  { title: '版本', dataIndex: 'mysqlVersion', key: 'mysqlVersion', width: 80 },
...
async function fetchList() {
  ...
  try {
    ...
    message.error('获取MySQL实例列表失败: ' + e.message);
  } finally {
    loading.value = false;
  }
}
...
function goDeploy() {
  router.push('/CLM/mysql/deploy');
}

async function handleCheckStatus(record: MySQLInstance) {
  try {
    await checkMySQLStatus(record.id);
...
async function handleStart(record: MySQLInstance) {
  try {
    const res = await startMySQL(record.id);
...
async function handleStop(record: MySQLInstance) {
  Modal.confirm({
    title: '确认停止',
    content: `确定要停止MySQL实例「${record.instanceName}」吗？`,
    async onOk() {
      try {
        const res = await stopMySQL(record.id);
...
async function handleRestart(record: MySQLInstance) {
  try {
    const res = await restartMySQL(record.id);
...
function handleShowLog(record: MySQLInstance) {
  deployLog.value = record.deployLog || '暂无部署日志';
  logModalVisible.value = true;
}

function handleDelete(record: MySQLInstance) {
  Modal.confirm({
    title: '确认删除',
    content: `确定要删除MySQL实例「${record.instanceName}」吗？该操作不可恢复。`,
    okType: 'danger',
    async onOk() {
      try {
        await deleteMySQLInstance(record.id);
          ...
          <template #bodyCell="{ column, record }">
            ...
            <template v-if="column.key === 'action'">
              <Space>
                ...
                <Button
                  v-if="
                    record.status === 'stopped' || record.status === 'deployed'
                  "
                  type="link"
                  size="small"
                ...
                <Button type="link" size="small" @click="handleShowLog(record)"
                  >日志</Button
                >
                <Button
                  type="link"
