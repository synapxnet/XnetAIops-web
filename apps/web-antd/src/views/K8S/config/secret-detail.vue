...
async function fetchData() {
  loading.value = true;
  try {
    info.value = await getSecret(clusterId, namespace, name);
  } catch (e: any) {
    message.error('获取Secret信息失败: ' + e.message);
  } finally {
    loading.value = false;
  }
}
...
function goBack() {
  router.push('/K8S/config/secret-list');
}
onMounted(fetchData);
</script>
...
<template>
  <BusinessPage
    ...
    family="详情"
    route-key="/K8S/config/secret-detail/:clusterId/:namespace/:name"
  >
    <div class="p-4">
      <Spin :spinning="loading">
        <Card class="mb-4">
          ...
          >
            <div>
              <h2 style="margin: 0">
                <Tag color="orange">Secret</Tag>{{ name }}
              </h2>
              <Space class="mt-1">
            ...
            <Card>
              <div v-if="info?.dataKeys && info.dataKeys.length > 0">
                ...
                <div style="margin-top: 12px; color: #8c8c8c; font-size: 12px">
                  Secret数据已Base64编码，出于安全考虑不直接显示内容
                </div>
              </div>
