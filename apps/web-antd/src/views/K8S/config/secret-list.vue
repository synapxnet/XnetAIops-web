...
const createData = ref<{ key: string; value: string }[]>([
  { key: '', value: '' },
]);
...
async function fetchData() {
  ...
  try {
    ...
    message.error('获取Secret列表失败: ' + e.message);
  } finally {
    loading.value = false;
  }
}
...
function goDetail(r: any) {
  router.push(
    `/K8S/config/secret-detail/${selectedClusterId.value}/${r.namespace}/${r.name}`,
  );
}
...
function handleDelete(r: any) {
  Modal.confirm({
    title: '确认删除',
    content: `确定要删除Secret「${r.name}」吗？`,
    okType: 'danger',
    async onOk() {
      try {
        await deleteSecret(selectedClusterId.value!, r.namespace, r.name);
...
<template>
  <BusinessPage
    ...
    family="列表"
    route-key="/K8S/config/secret-list"
  >
    <div class="p-4">
      ...
      >
        ...
        <div style="margin-bottom: 16px">
          ...
          <Select v-model:value="createType" style="width: 100%">
            <SelectOption v-for="t in secretTypeOptions" :key="t" :value="t">{{
              t
            }}</SelectOption>
          </Select>
        </div>
