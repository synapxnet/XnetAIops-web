...
const createData = ref<Array<{ key: string; value: string }>>([
  { key: '', value: '' },
]);
...
async function fetchData() {
  ...
  try {
    ...
    message.error('获取ConfigMap列表失败: ' + e.message);
  } finally {
    loading.value = false;
  }
}
...
function goDetail(r: any) {
  router.push(
    `/K8S/config/configmap-detail/${selectedClusterId.value}/${r.namespace}/${r.name}`,
  );
}
...
function handleDelete(r: any) {
  Modal.confirm({
    title: '确认删除',
    content: `确定要删除ConfigMap「${r.name}」吗？`,
    okType: 'danger',
    async onOk() {
      try {
        await deleteConfigMap(selectedClusterId.value!, r.namespace, r.name);
...
function removeDataEntry(idx: number) {
  createData.value.splice(idx, 1);
}
...
<template>
  <BusinessPage
    ...
    family="列表"
    route-key="/K8S/config/configmap-list"
  >
    <div class="p-4">
      ...
      >
        ...
        >
          ...
          />
          <Button
            v-if="createData.length > 1"
            size="small"
            danger
            @click="removeDataEntry(idx)"
            >删除</Button
          >
        </div>
