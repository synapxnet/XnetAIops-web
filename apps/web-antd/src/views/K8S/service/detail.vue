...
async function fetchData() {
  ...
  try {
    ...
    message.error('获取服务信息失败: ' + e.message);
  } finally {
    loading.value = false;
  }
}
...
<template>
  <BusinessPage
    ...
    family="详情"
    route-key="/K8S/service/detail/:clusterId/:namespace/:name"
  >
    <div class="p-4">
          ...
          <TabPane key="info" tab="基本信息">
            <Card title="服务属性" class="mb-4">
              <Descriptions bordered :column="2" size="small">
                ...
                <DescriptionsItem label="Cluster IP">{{
                  svcInfo?.clusterIP || '-'
                }}</DescriptionsItem>
            ...
            <Card v-if="svcInfo?.selector" title="选择器">
              <Tag v-for="(v, k) in svcInfo.selector" :key="k" color="blue"
                >{{ k }}={{ v }}</Tag
              >
            </Card>
          </TabPane>
              ...
              >
                ...
                <Table.Column title="就绪" key="ready">
                  <template #default="{ record }">
                    <Badge :status="record.ready ? 'success' : 'error'" />{{
                      record.ready ? '是' : '否'
                    }}
                  </template>
                </Table.Column>
