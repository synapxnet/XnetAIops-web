...
async function fetchData() {
  ...
  try {
    ...
    message.error('获取PV列表失败: ' + e.message);
  } finally {
    loading.value = false;
  }
}
...
function handleDelete(r: any) {
  Modal.confirm({
    title: '确认删除',
    content: `确定要删除PV「${r.name}」吗？`,
    okType: 'danger',
    async onOk() {
      try {
        await deletePV(selectedClusterId.value!, r.name);
        ...
        >
          <template #bodyCell="{ column, record }">
            ...
            <template v-if="column.key === 'accessModes'"
              ...
              ></template
            >
            <template v-if="column.key === 'claim'">{{
              record.claimNamespace
                ? record.claimNamespace + '/' + record.claimName
                : '-'
            }}</template>
