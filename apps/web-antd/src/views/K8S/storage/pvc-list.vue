...
async function fetchData() {
  ...
  try {
    ...
    message.error('获取PVC列表失败: ' + e.message);
  } finally {
    loading.value = false;
  }
}
...
function goCreate() {
  router.push(
    `/K8S/storage/pvc-create?clusterId=${selectedClusterId.value}&namespace=${selectedNamespace.value}`,
  );
}
...
function handleDelete(r: any) {
  Modal.confirm({
    title: '确认删除',
    content: `确定要删除PVC「${r.name}」吗？`,
    okType: 'danger',
    async onOk() {
      try {
        await deletePVC(selectedClusterId.value!, r.namespace, r.name);
...
<template>
  <BusinessPage
    ...
    family="列表"
    route-key="/K8S/storage/pvc-list"
  >
    <div class="p-4">
