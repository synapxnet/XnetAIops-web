...
function goPodDetail(record: any) {
  router.push(
    `/K8S/pod/detail/${clusterId}/${record.namespace}/${record.name}`,
  );
}
...
<template>
  <BusinessPage
    ...
    family="详情"
    route-key="/K8S/node/detail/:clusterId/:nodeName"
  >
    <div class="p-4">
      <Spin :spinning="loading">
        ...
        <Card class="mb-4">
          ...
          >
            <div>
              ...
              <Space class="mt-1">
                <Tag
                  v-if="nodeInfo?.status"
                  :color="nodeInfo.status === 'Ready' ? 'green' : 'red'"
                >
                  {{ nodeInfo.status }}
                </Tag>
                <Tag
                  ...
                  >{{ role }}</Tag
                >
                <Tag v-if="nodeInfo?.unschedulable" color="orange"
                  >不可调度</Tag
                >
              </Space>
            </div>
        ...
        <Tabs v-model:activeKey="activeTab">
          ...
          <TabPane key="info" tab="基本信息">
            <Card title="节点属性" class="mb-4">
              <Descriptions bordered :column="2" size="small">
                ...
                <DescriptionsItem label="操作系统">{{
                  nodeInfo?.systemInfo?.osImage || nodeInfo?.osImage || '-'
                }}</DescriptionsItem>
                <DescriptionsItem label="内核版本">{{
                  nodeInfo?.systemInfo?.kernelVersion ||
                  nodeInfo?.kernelVersion ||
                  '-'
                }}</DescriptionsItem>
                <DescriptionsItem label="容器运行时">{{
                  nodeInfo?.systemInfo?.containerRuntimeVersion ||
                  nodeInfo?.containerRuntime ||
                  '-'
                }}</DescriptionsItem>
                <DescriptionsItem label="Kubelet版本">{{
                  nodeInfo?.systemInfo?.kubeletVersion ||
                  nodeInfo?.kubeletVersion ||
                  '-'
                }}</DescriptionsItem>
                <DescriptionsItem label="架构">{{
                  nodeInfo?.systemInfo?.architecture ||
                  nodeInfo?.architecture ||
                  '-'
                }}</DescriptionsItem>
                <DescriptionsItem label="CPU容量"
                  >{{ nodeInfo?.cpuCapacity || '-' }} 核</DescriptionsItem
                >
                <DescriptionsItem label="内存容量">{{
                  formatMemory(
                    nodeInfo?.memoryCapacity,
                    nodeInfo?.memoryCapacityBytes,
                  )
                }}</DescriptionsItem>
                <DescriptionsItem label="Pod容量">{{
                  nodeInfo?.podCapacity || '-'
                }}</DescriptionsItem>
            ...
            <!-- 污点 -->
            <Card
              v-if="nodeInfo?.taints && nodeInfo.taints.length > 0"
              title="污点 (Taints)"
              class="mb-4"
            >
              <div
                v-for="(taint, idx) in nodeInfo.taints"
                :key="idx"
