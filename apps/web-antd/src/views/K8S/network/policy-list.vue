...
const columns = [
  ...
  {
    title: '入站规则',
    dataIndex: 'ingressRuleCount',
    key: 'ingressRuleCount',
    width: 100,
  },
  {
    title: '出站规则',
    dataIndex: 'egressRuleCount',
    key: 'egressRuleCount',
    width: 100,
  },
...
async function fetchData() {
  ...
  loading.value = true;
  try {
    const res = await getNetworkPolicies(
      selectedClusterId.value,
      selectedNamespace.value,
    );
    policies.value = Array.isArray(res) ? res : [];
  } catch (e: any) {
    message.error('获取网络策略列表失败: ' + e.message);
  } finally {
    loading.value = false;
  }
}
...
function handleDelete(r: any) {
  Modal.confirm({
    title: '确认删除',
    content: `确定要删除NetworkPolicy「${r.name}」吗？`,
    okType: 'danger',
    async onOk() {
      try {
...
<template>
  <BusinessPage
    ...
    family="列表"
    route-key="/K8S/network/policy-list"
  >
    <div class="p-4">
        ...
        >
          <template #bodyCell="{ column, record }">
            <template v-if="column.key === 'policyTypes'">
              <Tag
                v-for="t in record.policyTypes || []"
                :key="t"
                :color="t === 'Ingress' ? 'green' : 'orange'"
                >{{ t }}</Tag
              >
            </template>
