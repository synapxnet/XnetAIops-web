...
const resourceTiles = computed(() => [
  {
    label: 'Pod',
    count: overview.value?.podCount || 0,
    running: overview.value?.runningPods,
    color: '#1890ff',
    icon: '📦',
    path: `/K8S/pod/list?clusterId=${clusterId}&namespace=${namespaceName}`,
  },
  {
    label: 'Deployment',
    count: overview.value?.deploymentCount || 0,
    color: '#722ed1',
    icon: '⚙',
    path: `/K8S/workload/list?clusterId=${clusterId}&namespace=${namespaceName}`,
  },
  {
    label: 'StatefulSet',
    count: overview.value?.statefulSetCount || 0,
    color: '#13c2c2',
    icon: '🗄',
    path: `/K8S/workload/list?clusterId=${clusterId}&namespace=${namespaceName}&tab=statefulsets`,
  },
  {
    label: 'DaemonSet',
    count: overview.value?.daemonSetCount || 0,
    color: '#fa8c16',
    icon: '🔄',
    path: `/K8S/workload/list?clusterId=${clusterId}&namespace=${namespaceName}&tab=daemonsets`,
  },
  {
    label: 'Service',
    ...
    color: '#52c41a',
    icon: '🔗',
    path: `/K8S/service/list?clusterId=${clusterId}&namespace=${namespaceName}`,
  },
  {
    label: 'Job',
    count: overview.value?.jobCount || 0,
    color: '#eb2f96',
    icon: '⏱',
    path: `/K8S/job/list?clusterId=${clusterId}&namespace=${namespaceName}`,
  },
  {
    label: 'CronJob',
    count: overview.value?.cronJobCount || 0,
    color: '#faad14',
    icon: '🕐',
    path: `/K8S/job/list?clusterId=${clusterId}&namespace=${namespaceName}&tab=cronjobs`,
  },
  {
    label: 'ConfigMap',
    count: overview.value?.configMapCount || 0,
    color: '#2f54eb',
    icon: '📋',
    path: `/K8S/config/configmap-list?clusterId=${clusterId}&namespace=${namespaceName}`,
  },
  {
    label: 'Secret',
    count: overview.value?.secretCount || 0,
    color: '#595959',
    icon: '🔒',
    path: `/K8S/config/secret-list?clusterId=${clusterId}&namespace=${namespaceName}`,
  },
  {
    label: 'Ingress',
    count: overview.value?.ingressCount || 0,
    color: '#36cfc9',
    icon: '🌐',
    path: `/K8S/ingress/list?clusterId=${clusterId}&namespace=${namespaceName}`,
  },
]);
...
<template>
  <BusinessPage
    ...
    family="详情"
    route-key="/K8S/namespace/detail/:clusterId/:namespace"
  >
    <div class="p-4">
      <Spin :spinning="loading">
          ...
          >
            <div>
              <h2 style="margin: 0">
                ...
                <Tag
                  v-if="nsInfo?.status"
                  :color="nsInfo.status === 'Active' ? 'green' : 'red'"
        ...
        <Tabs v-model:activeKey="activeTab">
          ...
          <TabPane key="overview" tab="概览">
              ...
              >
                ...
                >
                  <div style="text-align: center">
                    ...
                    <div
                      style="font-size: 22px; font-weight: bold; margin: 4px 0"
            ...
            <Card
              v-if="quotaUsage.length > 0"
              title="资源配额使用"
              class="mb-4"
            >
              <div
                v-for="(item, idx) in quotaUsage"
                :key="idx"
            ...
            <Card title="快捷操作" class="mb-4">
              <Space wrap>
                <Button
                  ...
                  @click="
                    navigate(
                      `/K8S/workload/list?clusterId=${clusterId}&namespace=${namespaceName}`,
                    )
                  "
                  >查看工作负载</Button
                >
                <Button
                  @click="
                    navigate(
                      `/K8S/pod/list?clusterId=${clusterId}&namespace=${namespaceName}`,
                    )
                  "
                  >查看Pod</Button
                >
                <Button
                  @click="
                    navigate(
                      `/K8S/service/list?clusterId=${clusterId}&namespace=${namespaceName}`,
                    )
                  "
                  >查看服务</Button
                >
                <Button
                  @click="
                    navigate(
                      `/K8S/config/configmap-list?clusterId=${clusterId}&namespace=${namespaceName}`,
                    )
                  "
                  >查看配置</Button
                >
          ...
          <TabPane key="labels" tab="标签与注解">
            <Card title="标签 (Labels)" class="mb-4">
              <div
                v-if="nsInfo?.labels && Object.keys(nsInfo.labels).length > 0"
                style="display: flex; flex-wrap: wrap; gap: 6px"
              >
                <Tag
                  v-for="(value, key) in nsInfo.labels"
                  :key="key"
                  color="blue"
                  >{{ key }}={{ value }}</Tag
                >
            ...
            <Card title="注解 (Annotations)">
              <div
                v-if="
                  nsInfo?.annotations &&
                  Object.keys(nsInfo.annotations).length > 0
                "
                style="display: flex; flex-wrap: wrap; gap: 6px"
              >
                <Tag
                  v-for="(value, key) in nsInfo.annotations"
                  :key="key"
                  color="default"
                  >{{ key }}={{ value }}</Tag
                >
