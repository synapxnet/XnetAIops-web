...
async function fetchData() {
  ...
  try {
    ...
    message.error('获取RBAC数据失败: ' + e.message);
  } finally {
    loading.value = false;
  }
}
...
function handleDeleteCR(r: any) {
  Modal.confirm({
    title: '确认删除',
    content: `确定要删除ClusterRole「${r.name}」吗？`,
    okType: 'danger',
    async onOk() {
      try {
        await deleteClusterRole(selectedClusterId.value!, r.name);
...
function handleDeleteCRB(r: any) {
  Modal.confirm({
    title: '确认删除',
    content: `确定要删除ClusterRoleBinding「${r.name}」吗？`,
    okType: 'danger',
    async onOk() {
      try {
        await deleteClusterRoleBinding(selectedClusterId.value!, r.name);
        message.success('删除成功');
        fetchData();
      } catch (e: any) {
        message.error('删除失败: ' + e.message);
...
<template>
  <BusinessPage
    ...
    family="列表"
    route-key="/K8S/rbac/clusterrole-list"
  >
    <div class="p-4">
            ...
            >
              <template #bodyCell="{ column, record }">
                <template v-if="column.key === 'roleRef'">
                  <Tag color="blue"
                    >{{ record.roleRef?.kind }}: {{ record.roleRef?.name }}</Tag
                  >
                </template>
                <template v-if="column.key === 'subjectCount'">{{
