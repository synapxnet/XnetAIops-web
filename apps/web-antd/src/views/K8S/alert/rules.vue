...
function handleDelete(r: any) {
  Modal.confirm({
    title: '确认删除',
    content: `确定要删除告警规则「${r.name}」吗？`,
    okType: 'danger',
    async onOk() {
      try {
        await deleteAlertRule(selectedClusterId.value!, r.id);
    ...
    <div class="p-4">
      ...
      >
        <Form layout="vertical">
          ...
          <FormItem label="指标名称">
            <Input
              v-model:value="formData.metricName"
              placeholder="如: node_cpu_usage, pod_restart_count"
            />
          </FormItem>
          <div
