...
async function fetchData() {
  ...
  try {
    ...
    const metrics = await getApiServerMetrics(
    ...
    message.error('获取API Server监控数据失败: ' + e.message);
  } finally {
    loading.value = false;
  }
}
  ...
  if (Array.isArray(values)) {
    for (const item of values) {
      if (Array.isArray(item))
        pts.push([new Date(item[0] * 1000), Number(item[1])]);
    }
  }
  return pts;
}
...
function buildMultiSeriesOption(
  ...
  const allSeries = (seriesArr || []).map((s: any, i: number) => ({
    name:
      s[labelKey] || s.code || s.verb || s.request_kind || `Series ${i + 1}`,
    type: 'line' as const,
  ...
  return {
    ...
    tooltip: {
      ...
      formatter: (params: any) => {
        ...
        for (const p of params)
          html += `<br/>${p.marker} ${p.seriesName}: ${yFmt(p.value[1])}`;
        return html;
      },
    },
    ...
    grid: {
      ...
      right: 20,
      top: 35,
      bottom: allSeries.length > 1 ? 30 : 15,
    },
    xAxis: {
      type: 'time',
      axisLabel: {
...
function renderCharts(m: any) {
  ...
  init(
    ...
    buildMultiSeriesOption(
      '平均请求延迟',
      m.requestDuration
        ? [{ label: '延迟', values: m.requestDuration[0]?.values || [] }]
        : [],
      'label',
      (v) => v.toFixed(4) + ' s',
    ),
...
<template>
  <BusinessPage
    ...
    family="监控"
    route-key="/K8S/monitoring/api-server"
  >
    <div class="p-4">
      <Card title="API Server 监控">
        ...
        <Spin :spinning="loading">
          <Row :gutter="16" class="mb-4">
            <Col :span="12"
              ><Card size="small"
                ><div ref="reqRateRef" style="height: 280px" /></Card
            ></Col>
            <Col :span="12"
              ><Card size="small"
                ><div ref="latencyRef" style="height: 280px" /></Card
            ></Col>
          </Row>
          <Row :gutter="16">
            <Col :span="12"
              ><Card size="small"
                ><div ref="durationRef" style="height: 280px" /></Card
            ></Col>
            <Col :span="12"
              ><Card size="small"
                ><div ref="inflightRef" style="height: 280px" /></Card
            ></Col>
          </Row>
          <div
