...
async function fetchData() {
  try {
    ...
    message.error('获取计划详情失败: ' + e.message);
  } finally {
    loading.value = false;
  }
}
...
function goBack() {
  router.push(`/K8S/deploy/detail/${planId}`);
}
onMounted(fetchData);
...
<template>
  <BusinessPage
    ...
    family="表单"
    route-key="/K8S/deploy/add-node/:planId"
  >
    <div class="p-4">
      <Card
        :title="
          isCompleted
            ? `扩容节点 — ${detail?.plan?.planName || ''}`
            : `添加节点 — ${detail?.plan?.planName || ''}`
        "
      >
        <template #extra>
        ...
        <Card
          size="small"
          :title="
            isCompleted ? '添加Worker节点（将自动加入集群）' : '添加新节点'
          "
        >
            ...
            >
              <FormItem label="角色">
                <Select v-model:value="form.role" :disabled="isCompleted">
                  ...
                  <SelectOption v-if="!isCompleted" value="master"
                    >Master</SelectOption
                  >
                </Select>
              </FormItem>
