...
async function fetchData() {
  try {
    ...
    message.error('获取部署详情失败: ' + e.message);
  } finally {
    loading.value = false;
  }
}
...
function startPolling() {
  ...
  pollTimer = setInterval(() => {
    ...
    if (status === 'running') {
      ...
      const hasActiveNode = nodes.some(
        (n: any) =>
          n.status === 'pending' ||
          n.status === 'preparing' ||
          n.status === 'installing',
...
async function handleValidate() {
  try {
    ...
    if (res?.allValid) {
      ...
      message.warning({
        content: `${res?.valid}/${res?.total} 节点验证通过`,
        key: 'validate',
      });
    }
    fetchData();
  } catch (e: any) {
    message.error({ content: '验证失败: ' + e.message, key: 'validate' });
  }
}
...
function handleExecute() {
  Modal.confirm({
    title: '确认部署',
    content:
      '确定要开始部署Kubernetes集群吗？此操作将在所有节点上安装和配置组件。',
    async onOk() {
      try {
...
<template>
  <BusinessPage
    ...
    family="详情"
    route-key="/K8S/deploy/detail/:id"
  >
    <div class="p-4">
      <Spin :spinning="loading">
          ...
          >
            <div>
              <h2 style="margin: 0">
                ...
                <Tag
                  :color="statusColors[detail?.plan?.status] || 'default'"
            ...
            <Space>
              <Button
                v-if="detail?.plan?.status === 'pending'"
                @click="handleValidate"
                >验证节点</Button
              >
              <Button
                v-if="
                  detail?.plan?.status === 'pending' ||
                  detail?.plan?.status === 'validated' ||
                  detail?.plan?.status === 'completed'
                "
                @click="router.push(`/K8S/deploy/add-node/${planId}`)"
                >添加节点</Button
              >
              <Button
                v-if="
        ...
        >
          ...
          <div style="margin-top: 8px; color: #8c8c8c">
            节点就绪: {{ detail?.readyNodes || 0 }} /
            {{ detail?.totalNodes || 0 }}
            <span v-if="detail?.failedNodes > 0" style="color: #ff4d4f">
              (失败: {{ detail?.failedNodes }})</span
            >
          </div>
        </Card>
        ...
        <Card size="small" title="计划信息" class="mb-4">
          <Descriptions bordered :column="3" size="small">
            <DescriptionsItem label="K8s版本">{{
              detail?.plan?.k8sVersion
            }}</DescriptionsItem>
            <DescriptionsItem label="部署模式">{{
              detail?.plan?.deployType === 'ha' ? '高可用' : '单Master'
            }}</DescriptionsItem>
            <DescriptionsItem label="网络插件">{{
              detail?.plan?.networkPlugin
            }}</DescriptionsItem>
            <DescriptionsItem label="容器运行时">{{
              detail?.plan?.containerRuntime
            }}</DescriptionsItem>
            <DescriptionsItem label="Pod CIDR">{{
              detail?.plan?.podCidr
            }}</DescriptionsItem>
            <DescriptionsItem label="Service CIDR">{{
              detail?.plan?.serviceCidr
            }}</DescriptionsItem>
          </Descriptions>
        </Card>
          ...
          >
            <template #bodyCell="{ column, record }">
              <template v-if="column.key === 'role'">
                <Tag :color="record.role === 'master' ? 'blue' : 'green'">{{
                  record.role
                }}</Tag>
              </template>
          ...
          >
            <div v-for="log in logs" :key="log.id" style="margin-bottom: 4px">
              ...
              <span
                ...
                >[{{ log.logLevel }}]</span
              >
              <span v-if="log.nodeHost" style="color: #ce9178; margin-left: 8px"
                >[{{ log.nodeHost }}]</span
              >
