...
async function fetchData() {
  ...
  try {
    ...
    message.error('获取部署计划失败: ' + e.message);
  } finally {
    loading.value = false;
  }
}
...
function goCreate() {
  router.push('/K8S/deploy/create');
}
function goDetail(r: any) {
  router.push(`/K8S/deploy/detail/${r.id}`);
}

function handleDelete(r: any) {
  Modal.confirm({
    title: '确认删除',
    content: `确定要删除部署计划「${r.planName}」吗？`,
    okType: 'danger',
    async onOk() {
      try {
        await deleteDeployPlan(r.id);
        ...
        >
          <template #bodyCell="{ column, record }">
            <template v-if="column.key === 'planName'"
              ><a @click="goDetail(record)">{{ record.planName }}</a></template
            >
