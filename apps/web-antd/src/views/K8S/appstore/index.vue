...
async function fetchApps() {
  ...
  loading.value = true;
  try {
    const res = await searchHelmApps(
    ...
    message.error('搜索应用失败: ' + e.message);
  } finally {
    loading.value = false;
  }
}
...
function goDetail(app: any) {
  router.push({
    ...
    query: {
      ...
      repoId: app.repoId || selectedRepoId.value,
      chartName: app.name,
    },
  });
}
  ...
  >
    <div class="p-4">
      ...
      >
        ...
        >
          ...
          <Space>
            <Button
              type="primary"
              ghost
              @click="router.push('/K8S/appstore/releases')"
              >已安装应用</Button
            >
            <Button ghost @click="router.push('/K8S/appstore/repos')"
              >Helm 仓库</Button
            >
            <Segmented
              :value="mode"
              :options="[
                { label: '模板商店', value: 'templates' },
        ...
        <div
          v-if="mode === 'templates' && featuredTemplates.length > 0"
          style="margin-top: 24px; display: flex; gap: 16px; overflow-x: auto"
        >
          <div
            v-for="ft in featuredTemplates"
            :key="ft.id"
          ...
          >
            ...
            <div>
              ...
              <div
                ...
                "
              >
                {{ categoryLabels[ft.category] || ft.category }}
              </div>
            </div>
          </div>
        </div>
          ...
          <div style="flex: 1">
            <Spin :spinning="loading">
              <Row :gutter="[16, 16]">
                ...
                >
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        ...
                        <span
                          ...
                          style="
                            ...
                            font-weight: bold;
                          "
                          >{{ (tpl.displayName || tpl.name || '?')[0] }}</span
                        >
                      </div>
                      <div style="overflow: hidden">
                        ...
                        <Tag
                          size="small"
                          :color="
                            tpl.category === 'database'
                              ? 'blue'
                              : tpl.category === 'middleware'
                                ? 'orange'
                                : tpl.category === 'messaging'
                                  ? 'purple'
                                  : tpl.category === 'storage'
                                    ? 'cyan'
                                    : tpl.category === 'devops'
                                      ? 'green'
                                      : tpl.category === 'monitoring'
                                        ? 'red'
                                        : 'default'
                          "
                          >{{
                    ...
                    <div
                      class="text-muted-foreground min-h-[34px] overflow-hidden text-xs"
                      style="
                        display: -webkit-box;
                        -webkit-line-clamp: 2;
                    ...
                    >
                      <span style="font-size: 11px; color: #8c8c8c"
                        >{{ tpl.helmRepoName }}/{{ tpl.chartName }}</span
                      >
                      <Tag v-if="tpl.isFeatured" color="gold" size="small"
                        >推荐</Tag
                      >
                    </div>
                  </Card>
              ...
              <div
                ...
                style="text-align: center; color: #8c8c8c; padding: 60px"
              >
                暂无应用模板
              </div>
            </Spin>
        ...
        <Card class="mb-4">
          <Space>
            ...
            <Select
              ...
              @change="fetchApps"
            >
              <SelectOption v-for="r in repos" :key="r.id" :value="r.id">{{
                r.name
              }}</SelectOption>
            </Select>
            <Input.Search
          ...
          >
            ...
            >
              ...
              <div style="margin-top: 8px">
                <Tag v-if="app.repoName" size="small" color="blue">{{
                  app.repoName
                }}</Tag>
                <Tag
                  v-for="kw in (app.keywords || []).slice(0, 2)"
                  :key="kw"
                  size="small"
                  >{{ kw }}</Tag
                >
