...
async function fetchData() {
  ...
  try {
    ...
    message.error('获取仓库列表失败: ' + e.message);
  } finally {
    loading.value = false;
  }
}
...
function handleDelete(r: any) {
  Modal.confirm({
    title: '确认删除',
    content: `确定要删除仓库「${r.name}」吗？`,
    okType: 'danger',
    async onOk() {
      try {
        await deleteHelmRepo(selectedClusterId.value!, r.id);
        ...
        >
          <template #bodyCell="{ column, record }">
            ...
            <template v-if="column.key === 'action'">
              <Space>
                <Button type="link" size="small" @click="handleSync(record)"
                  >同步</Button
                >
                <Button
                  type="link"
        ...
        <Form layout="vertical">
          ...
          <FormItem label="仓库URL">
            <Input
              ...
              placeholder="如: https://charts.bitnami.com/bitnami"
            />
          </FormItem>
          <FormItem label="描述">
            <Input
