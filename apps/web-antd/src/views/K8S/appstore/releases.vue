...
async function fetchData() {
  ...
  try {
    ...
    message.error('获取Release列表失败: ' + e.message);
  } finally {
    loading.value = false;
  }
}
...
function handleUninstall(r: any) {
  Modal.confirm({
    title: '确认卸载',
    content: `确定要卸载Release「${r.releaseName}」吗？此操作不可撤销。`,
    okType: 'danger',
    async onOk() {
      try {
...
async function handleUpgrade() {
  if (!upgradeRelease.value || !upgradeVersion.value.trim()) {
    message.warning('请输入Chart版本');
    return;
  }
  ...
  try {
    await upgradeHelmRelease(
      selectedClusterId.value!,
      upgradeRelease.value.releaseName,
      {
...
async function handleRollback() {
  ...
  try {
    await rollbackHelmRelease(
      selectedClusterId.value!,
      rollbackRelease.value.releaseName,
      rollbackRelease.value.namespace,
...
<template>
  <BusinessPage
    ...
    family="列表"
    route-key="/K8S/appstore/releases"
  >
    <div class="p-4">
      <Card>
        <template #extra>
        ...
        >
          <template #bodyCell="{ column, record }">
            ...
            <template v-if="column.key === 'action'">
              <Space>
                <Button type="link" size="small" @click="openUpgrade(record)"
                  >升级</Button
                >
                <Button
                  type="link"
      ...
      >
        <div style="margin-bottom: 12px">
          <label>Release: </label
          ><strong>{{ upgradeRelease?.releaseName }}</strong>
          <span style="margin-left: 12px; color: #8c8c8c"
            >当前版本: {{ upgradeRelease?.chartVersion }}</span
          >
        </div>
        <div style="margin-bottom: 12px">
      ...
      >
        <div style="margin-bottom: 12px">
          <label>Release: </label
          ><strong>{{ rollbackRelease?.releaseName }}</strong>
          <span style="margin-left: 12px; color: #8c8c8c"
            >当前修订版: {{ rollbackRelease?.revision }}</span
          >
        </div>
        <div style="margin-bottom: 12px">
          <label>目标修订版号</label>
          <InputNumber
            v-model:value="rollbackRevision"
            :min="1"
            :max="(rollbackRelease?.revision || 2) - 1"
