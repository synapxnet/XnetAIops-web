...
async function fetchData() {
  ...
  loading.value = true;
  try {
    const res = await getIngresses(
      selectedClusterId.value,
      selectedNamespace.value,
    );
    ingresses.value = Array.isArray(res) ? res : [];
  } catch (e: any) {
    message.error('获取路由列表失败: ' + e.message);
  } finally {
    loading.value = false;
  }
}
...
function goCreate() {
  router.push(
    `/K8S/ingress/create?clusterId=${selectedClusterId.value}&namespace=${selectedNamespace.value}`,
  );
}
...
function handleDelete(r: any) {
  Modal.confirm({
    title: '确认删除',
    content: `确定要删除Ingress「${r.name}」吗？`,
    okType: 'danger',
    async onOk() {
      try {
        await deleteIngress(selectedClusterId.value!, r.namespace, r.name);
