...
const cpuPercent = computed(() => {
  ...
  return Math.round(
    (overview.value.cpuUsed / overview.value.cpuCapacity) * 100,
  );
});
const memoryPercent = computed(() => {
  if (!overview.value || !overview.value.memoryCapacity) return 0;
  return Math.round(
    (overview.value.memoryUsed / overview.value.memoryCapacity) * 100,
  );
});
const podPercent = computed(() => {
  if (!overview.value || !overview.value.podCapacity) return 0;
  return Math.round(
    (overview.value.podUsed / overview.value.podCapacity) * 100,
  );
});
const storagePercent = computed(() => {
  if (
    !overview.value ||
    !overview.value.storageCapacity ||
    !overview.value.storageUsed
  )
    return 0;
  return Math.round(
    (overview.value.storageUsed / overview.value.storageCapacity) * 100,
  );
});
...
const healthyComponents = computed(
  () => components.value.filter((c) => c.status === 'healthy').length,
);
...
const navTiles = computed(() => [
  {
    label: '节点',
    count: overview.value?.totalNodes || 0,
    icon: '🖥',
    color: '#1890ff',
    path: '/K8S/node/list',
  },
  {
    label: '命名空间',
    count: overview.value?.namespaceCount || 0,
    icon: '📁',
    color: '#722ed1',
  ...
  },
  {
    label: '工作负载',
    count:
      (overview.value?.deploymentCount || 0) +
      (overview.value?.statefulSetCount || 0) +
      (overview.value?.daemonSetCount || 0),
    icon: '⚙',
    color: '#13c2c2',
    path: `/K8S/workload/list?clusterId=${clusterId}`,
  },
  {
    label: 'Pod',
    count: overview.value?.totalPods || 0,
    icon: '📦',
    color: '#52c41a',
    path: `/K8S/pod/list?clusterId=${clusterId}`,
  },
  {
    label: '服务',
    count: overview.value?.serviceCount || 0,
    icon: '🔗',
    color: '#fa8c16',
    path: `/K8S/service/list?clusterId=${clusterId}`,
  },
  {
    label: '监控',
    count: null,
...
async function fetchData() {
  loading.value = true;
  try {
    const [overviewRes, componentsRes, eventsRes, rankingRes] =
      await Promise.all([
...
<template>
  <BusinessPage
    ...
    family="详情"
    route-key="/K8S/cluster/detail/:id"
  >
    <div class="p-4">
      <Spin :spinning="loading">
          ...
          >
            <div>
              <h2 style="margin: 0; font-size: 22px">
                ...
                <Tag
                  v-if="overview?.cluster?.status"
                  :color="
                    overview?.cluster?.status === 'active' ? 'green' : 'red'
                  "
                  style="margin-left: 8px; vertical-align: middle"
                >
                  {{
                    overview?.cluster?.status === 'active'
                      ? '运行中'
              ...
              <div style="display: flex; align-items: center; gap: 24px">
                ...
                <div>
                  ...
                  <div style="margin-bottom: 8px">
                    ...
                    <span style="font-weight: bold"
                      >{{ overview?.runningPods || 0 }} /
                      {{ overview?.totalPods || 0 }}</span
                    >
                  </div>
                </div>
              </div>
              ...
              <template #title>
                <Space>
                  ...
                  <span style="font-size: 13px; color: #8c8c8c"
                    >{{ healthyComponents }}/{{ totalComponents }} 健康</span
                  >
                </Space>
              </template>
              <Row :gutter="[12, 12]">
                ...
                >
                  <div
                    ...
                    :class="{
                      'component-healthy': comp.status === 'healthy',
                      'component-unhealthy': comp.status !== 'healthy',
                    }"
                  >
                    <div
                      style="
                    ...
                    >
                      ...
                      <Badge
                        :status="
                          comp.status === 'healthy' ? 'success' : 'error'
                        "
                        :text="comp.status === 'healthy' ? '健康' : '异常'"
                      />
                    </div>
                    <div
                      v-if="comp.message"
          ...
          <Row :gutter="16" justify="space-around">
            <Col :span="6" style="text-align: center">
              <Progress
                ...
                :percent="cpuPercent"
                :width="100"
                :stroke-color="cpuPercent > 80 ? '#ff4d4f' : '#1890ff'"
              />
              <div style="margin-top: 8px">
                <div style="font-size: 16px; font-weight: bold">
                  {{ formatCpu(overview?.cpuUsed || 0) }} /
                  {{ formatCpu(overview?.cpuCapacity || 0) }}
                </div>
            ...
            <Col :span="6" style="text-align: center">
              <Progress
                ...
                :width="100"
                :stroke-color="memoryPercent > 80 ? '#ff4d4f' : '#722ed1'"
              />
              <div style="margin-top: 8px">
                <div style="font-size: 16px; font-weight: bold">
                ...
                <div style="color: #8c8c8c">
                  内存 / {{ formatBytes(overview?.memoryCapacity || 0) }}
                </div>
              </div>
            </Col>
            <Col :span="6" style="text-align: center">
            ...
            <Col :span="6" style="text-align: center">
              <Progress
                ...
                :width="100"
                :stroke-color="storagePercent > 80 ? '#ff4d4f' : '#faad14'"
              />
              <div style="margin-top: 8px">
                <div style="font-size: 16px; font-weight: bold">
                ...
                <div style="color: #8c8c8c">
                  存储 / {{ formatBytes(overview?.storageCapacity || 0) }}
                </div>
              </div>
            </Col>
          </Row>
        ...
        <Card size="small" title="集群信息" class="mb-4">
          <Descriptions bordered :column="3" size="small">
            ...
            <DescriptionsItem label="K8s版本">{{
              overview?.k8sVersion || '-'
            }}</DescriptionsItem>
            <DescriptionsItem label="API Server">{{
              overview?.cluster?.apiServerUrl || '-'
            }}</DescriptionsItem>
            <DescriptionsItem label="提供商">{{
              overview?.cluster?.provider || '-'
            }}</DescriptionsItem>
            <DescriptionsItem label="网络插件">{{
              overview?.cluster?.networkPlugin || '-'
            }}</DescriptionsItem>
            <DescriptionsItem label="容器运行时">{{
              overview?.cluster?.containerRuntime || '-'
            }}</DescriptionsItem>
            <DescriptionsItem label="创建时间">{{
              overview?.cluster?.createdAt || '-'
            }}</DescriptionsItem>
            <DescriptionsItem label="Deployment">{{
              overview?.deploymentCount || 0
            }}</DescriptionsItem>
            <DescriptionsItem label="StatefulSet / DaemonSet"
              >{{ overview?.statefulSetCount || 0 }} /
              {{ overview?.daemonSetCount || 0 }}</DescriptionsItem
            >
          </Descriptions>
        </Card>
        ...
        <Card class="mb-4">
          <template #title>
            <Space>
              ...
              <Button size="small" @click="navigate('/K8S/node/list')"
                >查看全部</Button
              >
            </Space>
          </template>
            ...
            <template #bodyCell="{ column, record, index }">
              ...
              <template v-if="column.key === 'name'">
                <div>
                  <a
                    @click="
                      navigate(`/K8S/node/detail/${clusterId}/${record.name}`)
                    "
                    >{{ record.name }}</a
                  >
              ...
              <template v-if="column.key === 'cpu'">
                <div style="display: flex; align-items: center; gap: 8px">
                  <Progress
                    ...
                    style="width: 70px"
                    :stroke-color="
                      record.cpuPercent > 80 ? '#ff4d4f' : '#1890ff'
                    "
                  />
              ...
              <template v-if="column.key === 'memory'">
                <div style="display: flex; align-items: center; gap: 8px">
                  <Progress
                    ...
                    style="width: 70px"
                    :stroke-color="
                      record.memoryPercent > 80 ? '#ff4d4f' : '#722ed1'
                    "
                  />
                  <span style="font-size: 12px"
                    >{{ record.memoryPercent }}%</span
                  >
                </div>
              </template>
          ...
          >
            <template #bodyCell="{ column, record }">
              <template v-if="column.key === 'type'">
                <Tag :color="record.type === 'Warning' ? 'orange' : 'blue'">{{
                  record.type
                }}</Tag>
              </template>
        ...
        >
          <div style="margin-bottom: 12px; color: #8c8c8c; font-size: 13px">
            SSH 连接用于在集群 Master 节点上执行 Helm
            等命令，配置后即可使用应用商店一键部署功能。
          </div>
          <Form layout="vertical" :model="sshForm">
            <FormItem label="SSH 主机地址" required>
              <Input
                v-model:value="sshForm.sshHost"
                placeholder="集群Master节点IP，如 192.168.1.100"
              />
            </FormItem>
            <FormItem label="SSH 端口">
              <InputNumber
                v-model:value="sshForm.sshPort"
                :min="1"
                :max="65535"
                style="width: 120px"
              />
            ...
            <FormItem label="SSH 密码">
              <Input.Password
                v-model:value="sshForm.sshPassword"
                placeholder="与私钥二选一"
              />
            </FormItem>
            <FormItem label="SSH 私钥">
              <Textarea
                v-model:value="sshForm.sshKey"
                placeholder="PEM格式私钥（与密码二选一）"
                :rows="3"
