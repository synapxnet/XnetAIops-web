...
function goDetail(record: any, type: string) {
  router.push(
    `/K8S/job/detail/${selectedClusterId.value}/${record.namespace}/${type}/${record.name}`,
  );
}
            ...
            >
              <template #bodyCell="{ column, record }">
                ...
                <template v-if="column.key === 'status'">
                  <Tag :color="jobStatusColor[record.status] || 'default'">{{
                    record.status
                  }}</Tag>
                </template>
                ...
                <template v-if="column.key === 'action'">
                  <Space>
                    ...
                    <Button
                      ...
                      size="small"
                      danger
                      @click="handleDeleteCronJob(record)"
                      >删除</Button
                    >
                  </Space>
                </template>
