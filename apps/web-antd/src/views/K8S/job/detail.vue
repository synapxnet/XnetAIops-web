...
<template>
  <BusinessPage
    ...
    family="详情"
    route-key="/K8S/job/detail/:clusterId/:namespace/:type/:name"
  >
    <div class="p-4">
        ...
        <Card class="mb-4">
          ...
          >
            <div>
              ...
              <Space class="mt-1">
                <Tag :color="statusColor[info?.status] || 'default'">{{
                  info?.status || '-'
                }}</Tag>
            ...
            <Space>
              <Button v-if="type === 'Job'" @click="handleRerun"
                >重新执行</Button
              >
              <Button v-if="type === 'CronJob'" @click="handleTrigger"
                >手动触发</Button
              >
          ...
          <TabPane key="info" tab="基本信息">
            ...
            <Card v-if="type === 'Job'" title="Job属性" class="mb-4">
              <Descriptions bordered :column="2" size="small">
                ...
                <DescriptionsItem label="退避限制">{{
                  info?.backoffLimit ?? '-'
                }}</DescriptionsItem>
                <DescriptionsItem label="完成数">{{
                  info?.completions ?? '-'
                }}</DescriptionsItem>
                <DescriptionsItem label="并行度">{{
                  info?.parallelism ?? '-'
                }}</DescriptionsItem>
            ...
            <Card v-if="type === 'CronJob'" title="CronJob属性" class="mb-4">
              <Descriptions bordered :column="2" size="small">
                ...
                <DescriptionsItem label="暂停">{{
                  info?.suspend ? '是' : '否'
                }}</DescriptionsItem>
                <DescriptionsItem label="并发策略">{{
                  info?.concurrencyPolicy || '-'
                }}</DescriptionsItem>
                <DescriptionsItem label="成功保留数">{{
                  info?.successfulJobsHistoryLimit ?? '-'
                }}</DescriptionsItem>
                <DescriptionsItem label="失败保留数">{{
                  info?.failedJobsHistoryLimit ?? '-'
                }}</DescriptionsItem>
                <DescriptionsItem label="上次调度">{{
                  info?.lastScheduleTime || '-'
                }}</DescriptionsItem>
                <DescriptionsItem label="上次成功">{{
                  info?.lastSuccessfulTime || '-'
                }}</DescriptionsItem>
                <DescriptionsItem label="活跃Job数">{{
                  info?.activeJobs || 0
                }}</DescriptionsItem>
                <DescriptionsItem label="创建时间">{{
                  info?.createdAt || '-'
            ...
            <!-- 关联Pod（Job） -->
            <Card
              v-if="type === 'Job' && info?.pods?.length"
              title="关联Pod"
              class="mb-4"
            >
              <Table
            ...
            <!-- 最近Job（CronJob） -->
            <Card
              v-if="type === 'CronJob' && info?.recentJobs?.length"
              title="最近执行的Job"
            >
              <Table
